...
import { Home,BedDouble,UserCircle,Settings, Calendar,LogOut,ChevronLeft,ChevronRight,TableProperties, Hotel} from "lucide-react";
...
export default function Navbar({ sidebarOpen, setSidebarOpen }: Navbar) {
  const pathname = usePathname();
  return (
    // Sidebar container with dynamic width based on sidebarOpen state
    <div className={`fixed top-0 left-0 h-full transition-all duration-300 z-70 shadow-md border-r-2 border-blue-300 bg-sky-50
      ${sidebarOpen ? 'w-65' : 'w-28'}
    `}>
      ...
      <div className="h-15 p-6 flex items-center justify-between border-b-2 border-blue-300">
        <div className="flex items-center space-x-2">
          ...
          <span className={`font-bold text-xl text-blue-700 transition-opacity duration-300
